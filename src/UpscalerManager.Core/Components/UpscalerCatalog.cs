...
public static class UpscalerCatalog
{
    ...
    public static readonly IReadOnlyList<TechDefinition> All = new[]
    {
        ...
        new TechDefinition("amd_fidelityfx_upscaler_dx12.dll", "FSR (FidelityFX upscaler)", TechRole.Upscaler, "AMD",
            "AMD's upscaling module, introduced when FidelityFX SDK 2.0.0 split the runtime into separate effects. This is the file FSR 4 lives in, so its version is the FSR version you actually get — AMD's SDK 2.3.0 build of it is FSR 4.1.1."),
        ...
        new TechDefinition("amd_fidelityfx_framegeneration_dx12.dll", "FSR Frame Generation", TechRole.FrameGeneration, "AMD",
            "AMD's frame generation module; its version is the FSR frame generation version."),
        ...
        new TechDefinition("amd_fidelityfx_dx12.dll", "FidelityFX runtime (DX12)", TechRole.Runtime, "AMD",
            "Either a FidelityFX SDK 1 library with every effect inside it, or — from SDK 2 onwards — the loader under its old, deliberately compatible name. In the second case it holds no effect code, and its version is the SDK version rather than an FSR version."),
        new TechDefinition("amd_fidelityfx_loader_dx12.dll", "FidelityFX loader (DX12)", TechRole.Runtime, "AMD",
            "The FidelityFX SDK 2 loader. Contains no effect code — it loads the effect modules beside it — and carries the SDK version."),
