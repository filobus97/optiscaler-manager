...
///
/// AMD is absent because this app no longer swaps AMD's FidelityFX files — see
/// <see cref="SwappableDlls"/>. Worth recording that they <em>are</em> publishable: the
/// FidelityFX SDK is MIT and the signed SDK 2 modules are committed to
/// <c>FidelityFX-SDK</c> under <c>Kits/FidelityFX/signedbin/</c>, so if FSR swapping ever
/// comes back this is where it should fetch from. The reason it went away was never
/// availability; it was that the files changed shape between SDK generations and FSR 4
/// is not in any file most games ship.
/// </summary>
