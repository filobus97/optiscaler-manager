...
namespace UpscalerManager.Core.Tests
{
    ...
    public class AppDataMigrationTests : IDisposable
    {
        ...
        /// <summary>
        /// The mechanism is useless unless it is pointed at the name the app actually
        /// used before. This is the assertion that protects existing users' backups.
        /// </summary>
        [Fact]
        public void TheMigrationIsArmedWithTheNameTheAppUsedBefore()
        {
            SeedOldInstall("OptiscalerManager");
            var root = Path.Combine(_appData, AppDataPaths.FolderName);

            AppDataPaths.MigrateIfNeeded(_appData, root, AppDataPaths.PreviousFolderNamesForTests);
