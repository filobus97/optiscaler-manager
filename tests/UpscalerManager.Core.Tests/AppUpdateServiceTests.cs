...
namespace UpscalerManager.Core.Tests
{
    ...
    public class AppUpdateServiceTests
    {
        ...
        {
            var dir = PayloadDir("UpscalerManager", "update.sh", "config.json", "VERSION");
            try
            {
                Assert.Equal("UpscalerManager",
                    AppUpdateService.FindPayloadExecutableName(dir, "UpscalerManager"));
            }
        ...
        public void ARenamedPayloadIsFoundByElimination()
        {
            ...
            {
                Assert.Equal("UpscalerManager",
                    AppUpdateService.FindPayloadExecutableName(dir, "PreviousName"));
            }
            finally { Directory.Delete(dir, true); }
        }
        ...
        public void TheWindowsExecutableIsFound()
        {
            ...
            try
            {
                Assert.Equal("UpscalerManager.exe",
                    AppUpdateService.FindPayloadExecutableName(dir, "PreviousName.exe"));
            }
            finally { Directory.Delete(dir, true); }
        }
        ...
        public void AnAmbiguousPayloadIsNotGuessedAt()
        {
            ...
            try
            {
                Assert.Null(AppUpdateService.FindPayloadExecutableName(dir, "PreviousName"));
            }
            finally { Directory.Delete(dir, true); }
        }
        ...
        [Theory]
        [InlineData("UpscalerManager-0.9.0-linux-x64.zip", "linux-x64", true)]
        [InlineData("UpscalerManager-0.9.0-win-x64.zip", "win-x64", true)]
        [InlineData("UpscalerManager-0.9.0-osx-arm64.zip", "osx-arm64", true)]
        [InlineData("UpscalerManager-0.9.0-linux-x64.zip", "win-x64", false)]  // wrong platform
        [InlineData("UpscalerManager-0.9.0-osx-x64.zip", "osx-arm64", false)]  // arch mismatch
        ...
        public void SelectAssetUrl_PicksTheMatchingRid_AmongMany()
        {
            var assets = new[]
            {
                ("UpscalerManager-0.9.0-win-x64.zip", "u-win"),
                ("UpscalerManager-0.9.0-linux-x64.zip", "u-linux"),
                ("UpscalerManager-0.9.0-osx-arm64.zip", "u-mac"),
            };
